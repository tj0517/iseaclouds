    ...
    <div className="overflow-x-hidden">
      ...
      <div className="w-[100%] px-[5%] flex flex-row flex-wrap md:flex-nowrap bg-gray-400 pt-3 pb-9 justify-between text-7xl sm:text-6xl lg:text-7xl xl:text-8xl font-bold text-blue-800">
        {stats.map((stat, index) => (
          <motion.div key={index} variants={scaleUpDelay} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} custom={index + 1} className="w-[40%] sm:w-[20%]">
            <div className={`${playfair.className} text-blue-800 font-bold text-center`}>{stat.number}</div>
            <div className="text-white text-xl sm:text-lg md:text-xl lg:text-2xl mt-7 font-light text-center">{stat.label}</div>
          </motion.div>
        ))}
      </div>

      <div className="w-full px-10 md:px-20 xl:px-30 py-16 md:py-30 flex lg:flex-row flex-col justify-between">
        <motion.div className="flex-col w-[80%] ml-auto mr-auto xl:mr-0 xl:ml-0 lg:w-[40%] text-center lg:text-left" initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={fadeLeft}>
      ...
      <div className="w-full px-[2.5%] flex flex-col pt-10 pb-10">
        ...
        <div className="w-[100%] px-[5%] flex flex-row flex-wrap pb-9 justify-between mt-20 md:mt-30 text-[12px] md:text-[14px] xl:text-[16px]">
          {items.map((item, index) => (
            <motion.div key={index} custom={index + 1} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="w-[40%] lg:w-[20%] flex flex-col text-center mb-20 lg:mb-0">
              <div className={`${playfair.className} text-black text-2xl md:text-3xl xl:text-4xl font-bold pb-7 border-b-2 whitespace-pre-line`}>{item.title}</div>
