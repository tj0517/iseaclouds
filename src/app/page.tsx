...
<motion.h1
  className="text-blue-500 lg:text-blue-800 w-full lg:w-[70%] text-center lg:text-left text-4xl sm:text-5xl md:text-5xl lg:text-4xl xl:text-5xl 2xl:w-[75%] font-bold mt-16 md:mt-24 lg:mt-16"
  variants={fadeUp}
  initial="hidden"
  animate="visible"
  custom={1} // opóźnienie
...
<motion.div
  className="lg:w-[70%] text-center lg:text-left mt-12 lg:mt-8 xl:mt-15 text-amber-50 font-light text-lg xl:text-xl 2xl:w-[60%]"
  variants={fadeUp}
  initial="hidden"
  animate="visible"
  custom={2}
...
<motion.div
  className="p-4 bg-blue-800 text-white w-[40%] ml-[30%] lg:ml-0 mt-12 lg:mt-16 text-center rounded-full"
  variants={fadeUp}
  initial="hidden"
  animate="visible"
  custom={3}
...
<motion.div
  className="w-[40%] lg:w-[20%] ml-[30%] lg:ml-0 flex flex-row justify-between text-3xl lg:text-2xl xl:text-3xl text-white mt-[70px] lg:mt-[80px]"
  variants={fadeUp}
  initial="hidden"
  animate="visible"
  custom={4}
...
<div className="w-full flex flex-col md:flex-row pt-0 md:pt-20 lg:pt-28">
      <motion.div
        className="h-full w-full md:w-[45%] ml-[10%]"
        variants={fadeLeft}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
      >
        ...
        <div className="text-slate-400 w-[80%] text-[14px] md:text-[13.5px] lg:text-[15px] xl:text-[18px] mt-10 lg:mt-[50px] mb-16 md:mb-0">
          Sea Clouds was founded by a team of experts with over 25 years of
          professional experience and deep knowledge in the Offshore Oil & Gas
          and Wind Farm sectors. We specialize in delivering comprehensive
          offshore engineering solutions and technical advisory services across
          key disciplines including Marine, Offshore Construction, Survey and
          Seabed Intervention, as well as ROV (Remotely Operated Vehicle)
          operations. Our services are grounded in technical excellence, safety,
          and efficiency, ensuring that every involvement in our client’s project
          meets the highest industry standards. Partner with Sea Clouds for
          precise and dependable offshore engineering solutions—from concept
          design and planning to execution and support. Whether you require
          strategic offshore technical advisory or fully integrated engineering
          capabilities, we bring clarity and confidence to complex offshore
          challenges. Over...
        </div>
      </motion.div>
    ...
    <div className="w-full px-10 md:px-20 xl:px-30 py-16 md:py-30 flex lg:flex-row flex-col justify-between">
      <motion.div
        className="flex-col w-[80%] ml-auto mr-auto xl:mr-0 xl:ml-0 lg:w-[40%] text-center lg:text-left"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
        custom={0}
        variants={fadeLeft}
      >
        <div className="text-4xl xl:text-5xl text-blue-800 font-bold">
          Master Offshore Skills with SeaClouds Courses
        </div>
        ...
        <div className="bg-blue-800 px-4 py-2 text-xl text-amber-50 xl:w-[40%] w-[60%] ml-[20%] lg:ml-0 mt-10">
          Check out
        </div>
      </motion.div>
      ...
      <div className="w-[90%] md:w-[80%] lg:w-[50%] ml-auto mr-auto md:mr-0 md:ml-[10%] lg:ml-0 xl:w-[42.5%] flex flex-row justify-between flex-wrap h-full mt-20 lg:mt-0">
        {courses.map((src, index) => (
          <motion.div
            key={index}
            className="w-[47.5%] aspect-square relative mb-5"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.3 }}
            custom={index + 1}
      ...
      <div className="w-[100%] px-[5%] flex flex-row flex-wrap pb-9 justify-between mt-20 md:mt-30 text-[12px] md:text-[14px] xl:text-[16px]">
        {items.map((item, index) => (
          <motion.div
            ...
            viewport={{ once: true, amount: 0.3 }}
            variants={fadeUp}
            className={`w-[40%] lg:w-[20%] flex flex-col text-center mb-20 lg:mb-0`}
          >
            <div className={`${playfair.className} text-black text-2xl md:text-3xl xl:text-4xl font-bold pb-7 border-b-2 whitespace-pre-line`}>
              {item.title}
            </div>
            <div className="italic mt-6 xl:mt-10 text-slate-400">{item.text}</div>
          </motion.div>
        ))}
      </div>
    </div>
